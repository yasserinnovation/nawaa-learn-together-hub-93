...
const AdminDashboard = () => {
  ...
  const fetchAllData = useCallback(async () => {
    await Promise.all([fetchSpaces(), fetchCourses(), fetchTools(), fetchCompetitions(), fetchProfiles()]);
  ...
  if (
    ...
    loading ||
    (user && userRole === null) ||
    (user && !authLoading && isAdmin === false && userRole === null)
  ) {
    return (
      <Layout>
  ...
  if (user && !authLoading && userRole !== null && isAdmin === false) {
    return (
      <Layout>
  ...
  const handleEditSpace = async (updatedSpaceData: any) => {
    try {
      ...
      setSpaces(spaces.map((space) => (space.id === updatedSpaceData.id ? updatedSpace : space)));
        ...
        <main className="container mx-auto px-4 py-8 min-h-screen">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-2">Manage spaces, users, and platform settings</p>
            </div>
          ...
          >
            ...
            <Card className="hover-lift">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Avg rating: {stats.avgSpaceRating} stars</p>
              </CardContent>
            </Card>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <TabsContent value="overview" className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center py-2 border-t">
                        ...
                        <span className="font-semibold text-green-600">
                          {stats.estimatedRevenue.toLocaleString()} EGP
                        </span>
                      </div>
                    </div>
                  </CardContent>
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Security Note:</strong> Your Postgres database has security patches available. Please upgrade
                  your database in the Supabase dashboard to apply important security updates.
                </AlertDescription>
              </Alert>
            </TabsContent>
                        ...
                        <div key={space.id} className="flex items-center justify-between p-4 border rounded-lg">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="text-right mr-4">
                              {space.price_per_hour && <p className="font-medium">{space.price_per_hour} EGP/hr</p>}
                              <p className="text-sm text-gray-600">{space.equipment.length} equipment items</p>
                            </div>
                            <Button variant="outline" size="sm" onClick={() => setEditingSpace(space)}>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-4">
                              <div>
                                ...
                                <p className="text-sm text-gray-600">
                                  {course.category} • {course.age_group} • {course.duration_weeks} weeks •{" "}
                                  {course.level}
                                </p>
                              </div>
                              <Badge variant="secondary">{course.price} EGP</Badge>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-600 mb-4">
                        No competitions found. Add your first competition to get started.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {competitions.map((competition) => (
                        <div key={competition.id} className="flex items-center justify-between p-4 border rounded-lg">
                          <div className="flex-1">
                            <div className="flex items-center gap-4">
                              <div>
                                ...
                                <p className="text-sm text-gray-600">
                                  {competition.category} • {competition.age_group} •{" "}
                          ...
                          <div className="flex items-center gap-2">
                            <Button variant="outline" size="sm" onClick={() => handleDeleteCompetition(competition.id)}>
                              <Trash2 className="h-4 w-4" />
                            </Button>
                          </div>
                        </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  ...
                  <Badge variant="outline" className="bg-green-50 text-green-700">
                    Avg Rating:{" "}
                    {profiles.length > 0
                      ? (profiles.reduce((sum, p) => sum + (p.rating || 0), 0) / profiles.length).toFixed(1)
                      : "N/A"}
                  </Badge>
                </div>
              </div>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {profileStats?.top_cities ? (
                        Object.entries(profileStats.top_cities)
                          .slice(0, 5)
                          .map(([city, count]) => (
            ...
            <TabsContent value="settings" className="space-y-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-center py-8">
                    <p className="text-gray-600 mb-4">Platform settings and configuration options.</p>
          ...
          {/* Add Course Modal */}
          {showAddCourseForm && (
            <AddCourseForm onClose={() => setShowAddCourseForm(false)} onSubmit={handleAddCourse} />
          )}

          {/* Add Tool Modal */}
          {showAddToolForm && <AddToolForm onClose={() => setShowAddToolForm(false)} onSubmit={handleAddTool} />}

          {/* Add Competition Modal */}
          {showAddCompetitionForm && (
            <AddCompetitionForm onClose={() => setShowAddCompetitionForm(false)} onSubmit={handleAddCompetition} />
          )}

          {/* Add Space Modal */}
          {showAddForm && <AddSpaceForm onClose={() => setShowAddForm(false)} onSubmit={handleAddSpace} />}

          {/* Edit Space Modal */}
          {editingSpace && (
            <EditSpaceForm space={editingSpace} onClose={() => setEditingSpace(null)} onSubmit={handleEditSpace} />
          )}
        </main>
      </Layout>
    </>
