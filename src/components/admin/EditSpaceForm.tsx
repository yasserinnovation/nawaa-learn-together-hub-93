...
const EditSpaceForm = ({ space, onClose, onSubmit }: EditSpaceFormProps) => {
  ...
  // Initialize form data with space data
  useEffect(() => {
    if (space) {
      setFormData({
        name: space.name || "",
        ...
        coordinates: {
          lat: space.coordinates?.lat?.toString() || "",
          lng: space.coordinates?.lng?.toString() || ""
        },
        availability: {
          days: space.availability?.days || [],
          hours: space.availability?.hours || ""
        },
        images: space.images || []
      });
    }
